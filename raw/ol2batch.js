...
const t0 = Date.now();
const stale = ['gberr','gbnone','nomatch','lowres','tiny','badar','badar-reverted','olerr'];
const todo = books.filter(b => { const m = man[b.id]; return m && stale.includes(m.st); });
let saved = 0, tried = 0;

async function one(b) {
  try {
    const q = norm(b.t) + (b.a ? ' ' + norm(b.a) : '');
    const r = await fetch('https://openlibrary.org/search.json?q=' + encodeURIComponent(q) + '&fields=cover_i&limit=8&sort=editions');
